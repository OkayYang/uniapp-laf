	...
	<view>
				...
				<view class="special" style="display: flex;align-items: center;padding: 0 0 0 40rpx;">
					<view>
						<u-tag shape="circle" :text="info.relStatus==1?'失物招领':'寻物启事'"
							:type="info.relSatus==1?'primary':'warning'"></u-tag>
					</view>
		...
		<view class='comment'
			...
			<view v-if="comments.length!=0">
				<view class="comment_header" style="display: flex;justify-content: space-between;" @click="blur()">
					<view style="margin: 18rpx 0 0 40rpx;">
						<text>评论</text>
					</view>
				</view>
					...
					<view class="userComment" style="margin: -5rpx 0 15rpx 0;" @click="blur()">
						...
						<view class="comment_function"
							...
							<view class="reply"
								...
								<view class="custom-icon custom-icon-pinglun" style="color: #878a8f;font-size: 40rpx;">
								</view>
							</view>
						</view>
					</view>
					<view v-if="comment.replyComments!==null" class="reply_position"
						style="background-color: #f3f3f3;width: 85%;margin: 0 auto 15rpx auto;box-shadow: 0 0rpx 0rpx 0 rgba(0, 0, 0, 0.1), 0 0rpx 5rpx 0 rgba(0, 0, 0, 0.19);border-radius: 5rpx;">
						<view class="reply_details" style="margin: 0 0 0 20rpx;" v-for="reply in comment.replyComments">
							<view @touchend="touchend(reply,reply.comment.comId)" @touchmove="touchmove()"
								...
								<span v-else><span style="color:  #19a3ff;">{{reply.comm_user.nickName}}</span>回复<span
		...
		<u-tabbar style="display: flex;align-items: center;position:relative;top:0;" :border="false">
			<view v-if="!input.focus" class="unfocus-tabbar" style="display: flex;width: 100%;align-items: center;">
				...
				<tui-modal :show="renling.reShow" title="提示" content="确定取消认领吗？" :button="renling.button" shape="circle"
					@click="cancelAccept" :size="30" color="#606266"></tui-modal>
		...
		<tui-modal :show="loginPane.show" custom>
			<view class="tui-modal-custom">
				<view style="display: flex;justify-content: center;margin: -20rpx 0 20rpx 0;">
					<image src="/static/alert.png" class="tui-tips-img" style="width: 120rpx;height: 120rpx;"></image>
				</view>
