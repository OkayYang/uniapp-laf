<template>
    <view class="filter-wrapper" :style="{ height: height + 'rpx', top: top,'border-top':border?'1rpx solid #f2f2f2':'none' }" @touchmove.stop.prevent="discard" @ed="test">
        <view class="inner-wrapper">
            ...
            <view class="navs">
                <view class="c-flex-align" :class="{ 'c-flex-center': index > 0, actNav: index === actNav }" v-for="(item, index) in navData" :key="index" @click="navClick(index)" >
                    <view v-for="(child, childx) in item" :key="childx" v-if="child.select" style="font-size: 27rpx;">{{ child.text }}</view>
               ...
               <view class="date-wrapper">
                    <picker mode="date" @change="handleDate"  >
                        <view class="date c-flex-align" :style="{ height: height + 'rpx' }" @click="dateClick">
                            ...
                            <image v-if="!close" src="https://i.loli.net/2020/07/15/xjVSvzWcH9NO7al.png" mode="" class="icon-triangle" ></image>
							<view v-else class="custom-icon custom-icon-chacha" @click.stop="closeTime"></view>
                        </view>
                    </picker>
